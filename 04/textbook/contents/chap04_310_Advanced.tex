...
ファイル→「開く」メニューからホームディレクトリ下の「04」フォルダにある「3dsnake.hsp」を読み込んでください。

[F5]キーで実行すると「3Dスネークゲーム」が始まります。自分は\ruby{体}{からだ}をどんどん伸ばしながら進んでいく\ruby{緑色}{みどり|いろ}のスネーク(ヘビ)です。カーソルキーで好きな\ruby{方向}{ほう|こう}に\ruby{曲}{ま}がることができますが、自分の体や\ruby{壁}{かべ}に当たってしまうとゲームオーバーになってしまいます。

自分の体をうまく\ruby{伸}{の}ばして、赤く光っている\ruby{箱}{はこ}(\ruby{食料}{しょく|りょう})を食べることがゲームの\ruby{目的}{もく|てき}です。一定の\ruby{回数}{かい|すう}食料を食べると、次のレベルに\ruby{進}{すす}むことができます。どのレベルまで行けるか、\ruby{周}{まわ}りの人たちと\ruby{競争}{きょう|そう}してみましょう。


\begin{figure}[H]
    \begin{center}
      \includegraphics[keepaspectratio,width=10.834cm,height=7.514cm]{../../asset/image/s_3dsnake.png}
      \caption{3dsnake.hspの実行画面}
    \end{center}
\end{figure}
...
%2586
\newpage
\subsection{\useOmetoiExample　コイン\ruby{落}{お}としゲームを遊んでみよう}


\exampleinduction

たくさんの絵を一度に動かす例として、コイン落としゲームを遊んでみましょう。

スクリプトエディタの、ファイル→「開く」メニューから「tamafall.hsp」を読み込んで実行してみてください。

[F5]キーで実行すると、ゲーム画面が表示されます。

キャラクター(\ruby{珠音}{たま|ね}ちゃん)が、コインをどんどん落とすのでカーソルキーで左右に動かしてください。コインがゴール(\ruby{緑色}{みどり|いろ}の\ruby{四角}{し|かく})に入るとスコアが\ruby{加算}{か|さん}されます。

\begin{figure}[H]
    \begin{center}
      \includegraphics[keepaspectratio,width=10cm,height=10cm]{../../asset/image/s_tamafall.png}
      \caption{tamafall.hspの実行画面}
    \end{center}
\end{figure}

100個のコインが落とされるとゲームが終わります。たくさんのコインをゴールに入れるほどスコアが高くなります。うまく操作して高いスコアを\ruby{目指}{め|ざ}しましょう。ゲームを遊ぶだけではなく、改造できる場所も探してみましょう。


\newpage
\exampleanswer

プログラムを改造することで、面のデータを自分で変えることができます。

以下の場所を探して修正します。


\begin{lstlisting}[caption=tamafall.hspのプログラムの一部]
	mapd+="  0   0  "+cr
	mapd+="0   0   0"+cr
	mapd+=" 0 0 0 0 "+cr
	mapd+="0   0   0"+cr
...
このマップデータを修正することで、面の形が\ruby{変化}{へん|か}します。

\begin{itembox}[c]{マップデータの意味}
    「0」は▲(\ruby{釘}{くぎ})になります。「1」はゴールになります。空白部分は何もでません。自分だけの面を作ってみましょう。
\end{itembox}

文字は「”」で囲み、最後に「+cr」を入れるようにしてください。
