...
ジャンプアップゲームは、キャラクター(\ruby{珠音}{たま|ね}ちゃん)を\ruby{操作}{そう|さ}してコインを\ruby{取}{と}るゲームです。

\ruby{左右}{さ|ゆう}の\ruby{移動}{い|どう}とジャンプをさせることができます。
...
すべてのコインを取るとあなたの勝ちです。逆に、\ruby{地面}{じ|めん}に置かれている\ruby{矢}{や}に\ruby{当}{あ}たってしまうとゲームオーバーになってしまいます。
...
\begin{itembox}[c]{マップデータの意味}
    「0」はレンガ(\ruby{壁}{かべ})になります。「*」はコイン、「+」は矢になります。マップデータが書かれている場所を探して、自分だけの面を作ってみましょう。
\end{itembox}

でたらめに書き\ruby{直}{なお}してもエラーが出るだけです。\ruby{必}{かなら}ず、
...
文字は「”」で\ruby{囲}{かこ}み、\ruby{最後}{さい|ご}に「+cr」を入れます。

面のデータは、\ruby{縦}{たて}・\ruby{横方向}{よこ|ほう|こう}に\ruby{広}{ひろ}げることができます。


\exampleanswer

ゲームを改造することで、\ruby{難}{むずか}しくなったり、\ruby{簡単}{かん|たん}になったりします。
...
%1881
\newpage
\subsection{\useOmetoiExample　ゲームの\ruby{背景}{はい|けい}や絵を改造してみよう}
...
色を変更する場合は、ツールパレットの\ruby{描画色}{びょう|が|しょく}をクリックして、「描画色の変更」ダイアログを出してから好きな色を選びます。
