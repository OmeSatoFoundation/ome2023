...
\clearpage
\subsection{例題17　カメラの映像や画像をAIに認識させてみよう}

\subsubsection*{考え方}

ネットワークに接続した状態で、ターミナルから作業フォルダ(「02」)内にある「./install\_yolov8.sh」を実行すると、AIが\ruby{画像認識}{が|ぞう|にん|しき}を行う「YOLOv8」というプログラムを使えるようになります。

\begin{figure}[H]
    \begin{center}
        \includegraphics[keepaspectratio,width=8.881cm,height=7.011cm]{../../asset/image/s_bus.png}
        \caption{install\_yolov8.shの実行画面}
    \end{center}
\end{figure}

無事に準備が終わると、バスの前に人が立っている画像をAIが認識した結果が表示されます。AIが画像の中に何が写っているかを考えています。たとえば、人がいる部分が囲まれて「person(人)」になっているのがわかります。
このように認識された物体は英語で表示されるので、大人の人と一緒に何が認識されているか教えてもらいましょう。


\subsubsection*{例題17 答え}

ターミナルの画面を開いて、「cd \textasciitilde /02」と入力して実行した後で、「./install\_yolov8.sh」を実行してみましょう。

この準備には、ダウンロードするための時間がかかるのでしばらく待っていてください。
正しく動作したら、別な画像を認識させたり、WEBカメラの映像を認識させてみましょう。ターミナルから、2つのコマンドを実行させることができます。

\begin{description}
    \item 入力するコマンド \ \ \ \ \ \ \ 動作
    \item python test\_yolov8.py \ \ \ \ 「test.jpg」を認識させる
    \item python usbcam\_yolov8.py \ \ \ WEBカメラの映像を認識させる
\end{description}

ターミナルから「python test\_yolov8.py」を実行すると、「test.jpg」という画像ファイルを認識した結果が表示されます。「test.jpg」のファイルを別な画像に置き換えて試してみましょう。

また、WEBカメラをUSBに接続した状態で、ターミナルから「python usbcam\_yolov8.py」を実行するとWEBカメラの映像がそのまま認識されるようになります。これは、キーボードの「q」を押すと終了できます。カメラの前で色々と試してみながら、AIがどのように動作するか確かめてみましょう。

AIがWEBカメラの映像や画像ファイルの中に写っているものがわかると、どのようなことができるでしょう？　お家の人や先生と一緒に考えてみましょう。
