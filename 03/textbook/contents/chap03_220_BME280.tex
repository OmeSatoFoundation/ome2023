...
\code{init\_bme}\\
という命令で BME280 を初期化しています。この命令の\ruby{実行結果}{じっ|こう|けっ|か}が変数 \code{stat} に代入されています。初期化に失敗すると \code{stat} に 0 以外の値が入るので、次の if 文で初期化が成功したかどうかを\ruby{確認}{かく|にん}しています。\\
