...
ここから先は発展です。授業ではここまでを扱います。ここから先は、コマンドをもっと使ってみたい人のための内容です。パイプライン、フィルタコマンド、文字の置き換え、ファイルの検索などを学びます。

自習用のコマンドを試すときは、教材用のファイルで試しましょう。大事なファイルを使って試さないようにしましょう。\code{rm} や \code{rm -r} を使うときは特に注意してください。
...
\end{lstlisting}
この例では、\textasciitilde/03/rensyu/kokugo/syousetu.txtの中身の文字数・単語数・行数を表示しています。
