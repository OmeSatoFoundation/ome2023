...
デジタル入力\ruby{装置}{そう|ち}をHSPで使ってみましょう。GPIO24にボタン(\#103)をつけてください。HSPスクリプトエディタで\textasciitilde /05/digin.hspを開いて実行してみましょう。\\
\begin{figure}[H]
...
\code{if gpioin(24)=1 : goto *digital\_on}でデジタル入力があったとき\code{*digital\_on}に\ruby{移動}{い|どう}します。\code{*digital\_on}では \code{gpio 23, 1}でLEDを光らせています。デジタル入力がないときは \code{goto *digital\_off}で\code{*digital\_off}に\ruby{移動}{い|どう}します。\code{*digital\_off}では \code{gpio 23, 0}でLEDを消しています。
...
\addex{\ruby{傾斜}{けい|しゃ}センサー(\#110)をGPIO24につなげてdigin.hspを実行してみましょう。}
...
\addex{時間があまったら\ruby{挑戦}{ちょう|せん}してみましょう。\\ボタンを\ruby{押}{お}して、LEDのONとOFFが切りかわるように、digin.hspを書きかえましょう。}
\end{enumerate}
\end{tcolorbox}
\end{document}
