...
\begin{frame}
    ...
    \vspace{15pt}
    \begin{itemize}
        \item 赤外線を短い時間で点けたり消したりして、点滅のパターンで命令を表す
    \end{itemize}
    \begin{center}
...
\begin{frame}[fragile]
    ...
    \begin{lstlisting}[title=２つの信号を登録するときのtemplate.lircd.conf,label=２つの信号を登録するときのtemplate.lircd.conf]
    begin remote
    name fan
    flags RAW_CODES
    eps 30
...
\begin{frame}[fragile]
    \frametitle{赤外線送受信のまとめ}
    \begin{lstlisting}
    sudo service lircd stop
    mode2 -d /dev/lirc1 | tee <filename1>.txt
    convert_pattern <filename1>.txt > <filename2>.pattern
    template.lircd.conf を書きかえて <filename3>.lircd.conf を作成
    sudo cp <filename3>.lircd.conf /etc/lirc/lircd.conf.d/
    sudo service lircd restart
    irsend SEND_ONCE <リモコンの名前> <信号の名前>
    \end{lstlisting}
    \textpageref{35}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item なんとか.lircd.confファイルの"begin raw\_codes"の直後の"name"
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    \frametitle{センサー紹介}
    教科書37ページから51ページに詳しいセンサーの紹介があります。
\end{frame}
...
\begin{frame}
    ...
    教科書35ページ 問題 5-14
    \begin{itemize}
        \item \textcolor{red}{必ず家の人と一緒に行いましょう}
        \item \textcolor{red}{電子レンジ、IHコンロ、ストーブなどの発熱する家電や調理器具には使用しないでください}
        \item \textcolor{red}{安全に扱える家電だけを使いましょう}
